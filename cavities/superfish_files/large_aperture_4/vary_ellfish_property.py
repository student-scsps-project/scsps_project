...
def run_sweep(file_name, new_directory, property, values):
...
if __name__ == '__main__':
    ...
    values = np.linspace(8, 25, 18)
    run_sweep(file_name, new_directory, property, values)
    del(file_name)
    del(new_directory)
    del(property)
    del(values)
#
    ...
    values = np.linspace(0.1, 1.70, 18)
    run_sweep(file_name, new_directory, property, values)
    del(file_name)
    del(new_directory)
    del(property)
