...
The directory which is specified should contain all the {value for value in variable}FO files for a
...
def plot_all_results(ellfish_res, save=False, directory=None, percentage=False):
    ...
    if directory == None:
        ...
        directory = path.join(directory, 'percentage')
    ...
    for name, func in zip(a, b):
        ...
        if percentage == True:
            ...
            plt.plot(ellfish_res.parameter_list(), func(), '-s', linewidth=3)
            plt.ylabel(name)
...
if __name__ == '__main__':
    root_dir = 'transit_time_factor'

    # #%%
    # # Wall Angle
    # directory = "wall_angle/results/"
    # parameter_name = "Wall Angle (degree)"
    # wall_angle = ellfish_result(path.join(root_dir, directory), parameter_name)
    # del(directory)
    # del(parameter_name)
    ...
    # right_dome = ellfish_result(path.join(root_dir, directory), parameter_name)
    # del(directory)
    # del(parameter_name)
    ...
    # domeab = ellfish_result(path.join(root_dir, directory), parameter_name)
    # del(directory)
    # del(parameter_name)
    ...
    # irisab = ellfish_result(path.join(root_dir, directory), parameter_name)
    # del(directory)
    # del(parameter_name)

    # %%
    ## Bore Radius
    directory = "bore_radius/results/"
    parameter_name = "Bore Radius (cm)"
    borerad = ellfish_result(path.join(root_dir, directory), parameter_name)
    del(directory)
    del(parameter_name)


# %%
    # plot_all_results(wall_angle,    True, path.join(root_dir, 'plots', 'wall_angle'),  percentage=True)
    # plot_all_results(right_dome,    True, path.join(root_dir, 'plots/right_dome_b'),   percentage=True)
    # plot_all_results(domeab,        True, path.join(root_dir, 'plots/dome_a_over_b'),  percentage=True)
    # plot_all_results(irisab,        True, path.join(root_dir, 'plots/iris_a_over_b'),  percentage=True)
    plot_all_results(borerad, True, path.join(root_dir, 'plots/bore_radius'),      percentage=False)
